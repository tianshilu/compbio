...
for i in f:
    i=i.split('\t')
    i[3]=i[3].replace('\r\n','')
    if i[3]!='Direction'and int(i[3])<0:
        a=i[0]
        i[0]=i[1]
        i[1]=a
        i[3]=0-int(i[3])
...
for item in uni_list:
    ...
    for f in item:
        len_uni+=int(f[4])
    tlist.append('%s\t%02d\t%i\t%i'%('UNI',num,len(item)+1,len_uni+250))
